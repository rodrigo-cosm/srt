...
The Broadcast and Backup group types are predicted to solve the problem of
...
In case of broadcast, the socket group is used to collect multiple sockets,
...
**allows** the socket to accept bonded connections. Without this flag the
connection that attempts to be bonded will be rejected. The caller should
also require minimum version from the peer, otherwise it simply won't understand
the handshake extension information concerning bonding.
...
In order to connect with bonding, instead of creating a socket, you have
...
For broadcast type bonding then, you create a group this way:
...
of view is to make you aware of it. The bonding group, as managed, will
...
the bonding group, which allows then the payload to be retrieved in order.
...
or writing, the "sub-operation" on one of the bonded sockets might have
...
Currently implemented group types are Broadcast and Backup. For backup
