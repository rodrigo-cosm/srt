...
void FECFilterBuiltin::RcvRebuild(Group& g, int32_t seqno, Group::Type tp)
{
    ...
    {
        LOGC(pflog.Error, log << "FEC: DECLIPPED length '" << length_hw << "' exceeds payload size. NOT REBUILDING.");
        return;
    }
...
int FECFilterBuiltin::ExtendRows(int rowx)
{
    ...
    {
        LOGC(pflog.Error, log << "FEC/H: OFFSET=" << rowx << " exceeds maximum row container size, SHRINKING rows and cells");
...
int FECFilterBuiltin::ExtendColumns(int colgx)
{
    if (colgx > int(sizeRow() * 2))
    {
        ...
        LOGC(pflog.Error, log << "FEC/V: OFFSET=" << colgx << " exceeds maximum col container size, SHRINKING container by " << sizeRow());
